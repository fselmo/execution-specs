...
def test_gas_refunds_from_data_floor(
    ...
    gas_after_refund = gas_before_refund - refund
    if (
        refund_test_type
        == RefundTestType.EXECUTION_GAS_MINUS_REFUND_LESS_THAN_DATA_FLOOR
    ):
        assert gas_after_refund < tx_floor_data_cost
    elif (
        refund_test_type
        == RefundTestType.EXECUTION_GAS_MINUS_REFUND_GREATER_THAN_DATA_FLOOR
    ):
        ...
        raise ValueError("Invalid refund test type")
    if gas_after_refund < tx_floor_data_cost:
        gas_after_refund = tx_floor_data_cost
    ...
    if fork >= Amsterdam:
        tx.expected_receipt = TransactionReceipt(
            gas_used=gas_before_refund,
            gas_spent=gas_after_refund,
        )
    else:
        tx.expected_receipt = TransactionReceipt(gas_used=gas_after_refund)
