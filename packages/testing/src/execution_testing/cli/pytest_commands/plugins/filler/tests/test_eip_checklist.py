...
def test_eip_checklist_generated_test(testdir: Any) -> None:
    """
    Test that a test built by a generator defined outside the EIP directory
    is attributed to the EIP of the module that collects it.
    """
    tests_dir = testdir.mkdir("tests")
    generators_module = tests_dir.join("generators.py")
    generators_module.write(
        textwrap.dedent(
            """
            import pytest
            from execution_testing import StateTestFiller
    ...
    test_7702_module.write(
        textwrap.dedent(
            ...
            from generators import generate_test
