...
class T8N(Load):
    ...
    def _run_blockchain_test(self, block_env: Any, block_output: Any) -> None:
        ...
        # EIP-7928: Set transaction index for BAL tracking
        ...
        for i, tx in zip(self.txs.successfully_parsed, self.txs.transactions):
            self.backup_state()
            try:
                # EIP-7928: Set transaction index for BAL tracking (1 to n)
                if self.fork.is_after_fork("ethereum.osaka"):
        ...
        if self.fork.is_after_fork("ethereum.shanghai"):
            # EIP-7928: Set post-execution index for BAL tracking
            if self.fork.is_after_fork("ethereum.osaka"):
            ...
            self.fork.process_withdrawals(
                block_env, block_output, self.env.withdrawals, change_tracker
            )

        if self.fork.is_after_fork("ethereum.prague"):
            self.fork.process_general_purpose_requests(
                block_env, block_output, change_tracker
            )
